...
\section{Igazolások és modellek}

As you might already suspect, there is a connection between \emph{theorems} and \emph{tautologies}.

Ezt ahogy már alapból sejthetted, van kapcsolat a \emph{theorems} és a \emph{tautologies} között.
...
Van egy formális út, hogy bemutassuk, hogy a tétel egy theorem: Bizonyítsd be. Az összes sorra, letudjuk ellenőrizni, hogy az adott sor követi-e a hivatkozott szabályt. Nehéz lehet létrehozni egy 20 soros bizonyítást, viszont nem nehéz ellenőrizni az összes sor bizonyosságát, és megerősíteni, hogy az helyes --– és,ha az összes sor a bizonyításból egyénileg helyes, akkor az egész bizonyítás helyes. Bemutatni, hogy a tautology egy tautológia,nehéz, az angolban megköveteli az okokat az összes lehetséges modellektől. Itt nincsen formális út, hogy leellenőrizzük, hogy lássuk, ha az érvelés megalapozott. Esélyt adva aközött, hogy bemutassuk a tétel egy theorem és, hogy bemutassuk ez egy tautology, az egyszerűbb lenne, ha bemutatnánk, hogy ez egy theorem.
...
Ellenben, nincs formális út bemutatni, hogy a tétel az \emp{not} egy theorem. Az angolban igazolnunk kell az összes lehetséges bizonyítást. Míg van formális metódus, hogy bemutassuk a tétel az nem egy tautology. Nekünk csak kreálnunk kell egy modellt amiben a tétel hamis. Választást adni aközött, hogy bemutassuk a tétel az nem egy theorem, és aközött, hogy nem egy tautology, egyszerűbb lenne bemutatni, hogy ez nem tautology.
...
Szerencsére a tétel egy theorem akkor és csak akkor, ha az egy tautology. Ha bizonyítékot szolgáltatunk $\vdash\script{A}$ és így bemutatjuk, hogy ez egy theorem, abból az következik, hogy \script{A} egy tautology; i. e., $\models\script{A}$. Hasonlóképpen, ha konstrutálunk egy modellt ahol \script{A} hamis és így bemutatjuk, hogy ez nem tautology, abból az következik, hogy \script{A} nem szabály.

In general, $\script{A}\vdash\script{B}$ if and only if $\script{A}\models\script{B}$. As such:
Általánosságban $\script{A}\vdash\script{B}$ akkor és csak akkor ha $\script{A}\models\script{B}$ Mint olyan:
\begin{itemize}
\item An argument is \emph{valid} if and only if \emph{the conclusion is derivable from the premises}.

\item Az érv \emph{valid} akkor és csak akkor ha \emph{the conclusion is derivable from the premises}.

\item Two sentences are \emph{logically equivalent} if and only if they are \emph{provably equivalent}.

\item Két tétel \emph{logically equivalent} akkor és csak akkor ha \emph{provably equivalent}.

\item A set of sentences is \emph{consistent} if and only if it is \emph{not provably inconsistent}.

\item Egy készlet tétel \emph{consistent} akkor is csak akkor ha az \emph{not provably inconsistent}.
...
Kiválaszthatod, hogy mikor szeretnél gondolkozni a bizonyítások szempontjából, és mikor a modellek szempontjából, Csinálva bármelyik is egyszerűbb az adott feladathoz. A \ref{table.ProofOrModel} táblázat összefoglalja, hogy mikor a legjobb bizonyításokat adni, és mikor a legjobb modelleket adni.
...
Ezen az úton a bizonyítások és a modellek sokoldalú eszközkészletet adnak az érvekkel való munka érdekében. Ha letudunk fordítani egy érvelést QL-be, akkor mértékelni tudjuk, hogy logikai súlya van tisztán formális úton. Ha ez deduktív érvényes, formális érvelést tudunk adni; Ha ez érvénytelen akkor ellenpéldát tudunk biztosítani. 
